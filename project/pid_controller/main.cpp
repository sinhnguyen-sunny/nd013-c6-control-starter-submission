...
BehaviorPlannerFSM behavior_planner(P_LOOKAHEAD_TIME, P_LOOKAHEAD_MIN, P_LOOKAHEAD_MAX, P_SPEED_LIMIT,
                                    P_STOP_THRESHOLD_SPEED, P_REQ_STOPPED_TIME, P_REACTION_TIME, P_MAX_ACCEL,
                                    P_STOP_LINE_BUFFER);
...
void path_planner(vector<double>& x_points, vector<double>& y_points, vector<double>& v_points, double yaw,
    ...
    if (x_points.size() > 1) {
        ego_state.rotation.yaw = angle_between_points(x_points[x_points.size() - 2],
                                                      y_points[y_points.size() - 2], x_points[x_points.size() - 1],
    ...
    for (int i = 0; i < spirals.size(); i++) {
        auto trajectory = motion_planner._velocity_profile_generator.generate_trajectory(spirals[i], desired_speed,
...
int main() {
    ...
    PID pid_steer = PID();
    pid_steer.Init(0.3, 0.0011, 0.9, 1.2, -1.2);

    // initialize pid throttle
    PID pid_throttle = PID();
    pid_throttle.Init(0.2, 0.0001, 0.04, 1.0, -1.0);
    ...
    h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char* data, size_t length, uWS::OpCode opCode) {
        ...
        if (s != "") {
            ...
            path_planner(x_points, y_points, v_points, yaw, velocity, goal, is_junction, tl_state, spirals_x,
                         spirals_y, spirals_v, best_spirals);
            ...
            double min_metric = 1e5;
            int min_idx;
            // Compute the desired yaw
            for (int i = 0; i < x_points.size(); ++i) {
                double metric = pow((x_position - x_points[i]), 2) + pow((y_position - y_points[i]), 2);
            ...
            double error_steer = angle_between_points(x_position, y_position, x_points[min_idx], y_points[min_idx]) - yaw;
            ...
            // Compute the throttle error (error_throttle) from the position and the desired speed
            error_throttle = v_points[min_idx] - velocity;
